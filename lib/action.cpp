...
void Action::play(std::string arg)
{
    if (arg == "")
    {
        system(("ffplay " + this->filename).c_str());
    }
    else if (arg.at(0) == '-')
    {
    ...
    {
        system(("ffplay -ss " + arg + " " + this->filename).c_str());
    }
}
...
//void Action::concat(std::string inputlist, std::string ofilepath)
//{
//    system(("ffmpeg -f concat -safe 0 -i \"" + inputlist + "\" -c copy \"" + ofilepath + "\"").c_str());
//}

int Action::concat(const std::string& inputlist, const std::string& ofilepath) {
    std::stringstream command;
    command << "ffmpeg -f concat -safe 0 -i \"" << inputlist << "\" -c copy \"" << ofilepath << "\"";

    // Open a pipe to read the command output
    FILE* pipe = popen(command.str().c_str(), "r");
    if (!pipe) {
        std::string errorMsg = "Error occurred during the execution of ffmpeg command";
